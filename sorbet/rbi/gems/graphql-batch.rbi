...
class GraphQL::Batch::Loader
  ...
  def resolve; end
  def resolved?; end
  def self.for(*group_args); end
  def self.load(key); end
  def self.load_many(keys); end
  def self.loader_key_for(*group_args); end
  def wait; end
end
...
class GraphQL::Batch::Setup
  ...
  def self.end_batching; end
  def self.instrument_field(schema, type, field); end
  def self.start_batching(executor_class); end
end
